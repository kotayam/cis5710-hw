...
module DatapathMultiCycle (
	...
	reg illegal_insn;
	reg [3:0] div_stage;
	reg [3:0] div_stage_next;
	reg in_div_state;
	reg in_div_state_next;
	always @(posedge clk)
		if (rst) begin
			div_stage <= 0;
			in_div_state <= 0;
	...
	always @(*) begin
		...
		else if (insn_uses_divider) begin
			in_div_state_next = 1;
			div_stage_next = 0;
		end
		if (in_div_state_next)
			pcNext = pcCurrent;
		case (insn_opcode)
			OpLui: begin
				we = 1'b1;
	...
	assign trace_completed_cycle_status = (in_div_state_next ? 32'd2 : 32'd1);
	initial _sv2v_0 = 0;
endmodule
